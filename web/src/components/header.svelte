...
<div
    class="top-0 h-8 dark:bg-header bg-header-light dark:text-white text-black pl-1.5 pt-1"
>
    <p
    ...
    >
        <option
            value="examples"
            disabled
            class="dark:bg-sidebar bg-sidebar-light">Examples</option
        >
        {#each examples as e}
            <optgroup label={e.name} class="dark:bg-sidebar bg-sidebar-light">
                {#each e.examples as e}
                    <option value={e} class="dark:bg-sidebar bg-sidebar-light"
                        >{e.name}</option
                    >
                {/each}
            </optgroup>
    ...
    <select
        bind:value={subMenu}
        on:change={subMenuEvent}
        class="inline border-none bg-transparent outline-none appearance-none cursor-pointer w-12"
    >
        <option
            value="subMenu"
            disabled
