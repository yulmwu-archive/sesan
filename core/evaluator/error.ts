...
export const printError = (error: Sesan.ParseError, file: string, stderr: Sesan.Stdio, options: Sesan.Options) => {
    ...
    stderr(
        `${options.stderrPrefix ? `${options.stderrColor ? `[Error]`.bgRed : `[Error]`} ` : ''}${options.stderrColor ? message.red : message} (${
            options.stderrColor ? `${file}:${`${line}:${column}`.yellow}` : `${file}:${line}:${column}`
        })`
    )
}
